...
export function EditUserModal({ loadedUser, setLoadedUser, setShowModal }) {
  ...
  function uploadDisplayPicture() {
    ...
    function handleUploadError(error) {
      setUploadError(
        `Oops, something went wrong: ${JSON.stringify(error)}. Please try again later.`
      )
    }
    ...
    const uploadTask = uploadBytesResumable(
      ref(storage, `displayPictures/${loadedUser.uid}`),
      selectedFile
    )
  ...
  async function updateUser(newURL) {
    ...
    try {
      ...
      if (response?.ok) {
        ...
        setUploadError(
          `Oops, something went wrong. We were unable to update the user. Please try again later.`
        )
      }
    } catch (error) {
      setUploadError(
  ...
  return (
    <Modal title="Edit User" onClose={() => setShowModal(false)}>
      {uploadStatus ? (
        ...
        <>
          <h2 className="mb-4 text-xl">Change Display Picture</h2>
          ...
          <div className="my-4 flex w-full flex-col">
            <h2 className="mb-4 text-xl">Change Display Name</h2>
            <Input
