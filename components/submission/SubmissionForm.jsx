...
export function SubmissionForm({ setResult, setShowResultModal }) {
  ...
  const [colour, setColour] = useState(
    colourChoices.find((c) => c.value === submission?.colour) || colourChoices[0]
  )
  const [name, setName] = useState(
    submission?.name ? { value: submission.name, label: submission.name } : ''
  )
  ...
  async function handleSubmit() {
    ...
    const newSubmission = {
      ...
      contributorDisplayName: user?.uid
        ? userData?.displayName || 'A registered user'
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="panel-select mb-4 text-black">
        <Select
          id="type-form-input"
      ...
      {contributionType?.value.includes('Trap') && (
        <div className="mb-4">
          ...
          <div className="panel-select text-black">
            <Select
              id="type-form-input"
          ...
          <ErrorMessage>
            <div className="flex flex-wrap items-center">
              <div className="w-full sm:w-3/4">
                Note: Submitting without being logged in means your contribution will be submitted
                anonymously and you will not get credit for your contribution. Your submission is
                automatically saved for when you come back.
              </div>
              <div className="mt-4 w-full sm:mt-0 sm:w-1/4 sm:pl-4">
