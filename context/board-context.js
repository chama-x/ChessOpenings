...
export const ChessboardProvider = ({ children }) => {
  ...
  useEffect(() => {
    setMoveSounds({
      ...
      capture: soundsOn?.value
        ? new Audio(`/media/themes/default/capture.mp3`)
        : { play: () => null },
      check: soundsOn?.value ? new Audio(`/media/themes/default/check.mp3`) : { play: () => null },
      castle: soundsOn?.value
        ? new Audio(`/media/themes/default/castle.mp3`)
        : { play: () => null },
  ...
  function resume() {
    ...
    if (game?.history({ verbose: true })[game.history().length - 1]?.color === userColor[0]) {
      makeComputerMove(
        opening.value[game.history().length].from,
        opening.value[game.history().length].to
      )
    } else {
